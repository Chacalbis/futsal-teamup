...
class TeamBalancer:
    ...
    def __init__(self, players, num_teams, num_players_per_team, config):
        ...
        self.weight_balance = config.get('weight_balance', 0.3)
        self.weight_profile = config.get('weight_profile', 0.5)
        self.weight_variance = config.get('weight_variance', 0.2)
...
def main():
    ...
    if missing_players:
        print(f"Erreur : Les joueurs suivants sont inscrits dans Google Sheets mais absents de {config['players_file_path']} : {', '.join(missing_players)}")
        return
